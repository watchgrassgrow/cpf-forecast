...
import { ChipGroup, SliderField, NumberField, SectionLabel, Toggle } from './Primitives';
import LifeEventsStep from './LifeEventsStep';
...
const TOTAL_STEPS = 5;
const STEP_LABELS = ['Profile', 'Balances', 'Retirement', 'Life Events', 'Assumptions'];
      ...
      <div className={styles.body}>
        ...
        {step === 1 && (
          <>
            ...
            {/* CPFIS section */}
            <div className={styles.cpfisDivider} />
            <SectionLabel>CPFIS — OA investments (optional)</SectionLabel>
            <div className={styles.field}>
              <Toggle
                checked={form.cpfisEnabled}
                onChange={v => set({ cpfisEnabled: v })}
                label="I use CPFIS to invest my OA savings"
              />
            </div>
            {form.cpfisEnabled && (
              <>
                <NumberField
                  label="Current CPFIS market value"
                  value={form.cpfisBalance}
                  step={1000} prefix="$"
                  onChange={v => set({ cpfisBalance: v })}
                  tip={form.balOA < 20000
                    ? '⚠ OA is below $20,000 — no funds are currently investable under CPFIS'
                    : `Investable OA headroom: $${Math.max(0, form.balOA - 20000).toLocaleString()} (OA − $20,000 floor)`}
                />
                <NumberField
                  label="Monthly OA → CPFIS contribution"
                  value={form.cpfisMonthlyContrib}
                  step={100} prefix="$"
                  onChange={v => set({ cpfisMonthlyContrib: v })}
                  tip="Amount transferred from OA to CPFIS each month (like a Regular Savings Plan)"
                />
                <SliderField
                  label="Expected annual return"
                  value={form.cpfisAnnualReturn}
                  min={-10} max={15} step={0.5}
                  format={v => `${v > 0 ? '+' : ''}${v.toFixed(1)}%`}
                  onChange={v => set({ cpfisAnnualReturn: v })}
                  tip="Returns compound annually. Can be negative — CPFIS carries market risk."
                />
                <div className={styles.field}>
                  <label className={styles.label}>Stop new contributions at age</label>
                  <ChipGroup
                    options={[
                      { label: '50', value: 50 },
                      { label: '55', value: 55 },
                    ...
                    value={form.cpfisStopAge}
                    onChange={v => set({ cpfisStopAge: v as number })}
                  />
                  <p className={styles.tip}>At 55, CPF LIFE assessment occurs — most members stop or sell CPFIS investments</p>
                </div>
                <div className={styles.cpfisWarning}>
                  ⚠ <strong>CPFIS carries market risk.</strong> CPF research found ~45% of investors
                  made losses over 2013–2022. Fees are not modelled here. Returns are not guaranteed.
                  The OA guaranteed rate (2.5%) is shown as a benchmark comparison in results.
                </div>
              </>
            )}
          </>
        ...
        {step === 3 && (
          <LifeEventsStep form={form} onFormChange={set} />
        )}
