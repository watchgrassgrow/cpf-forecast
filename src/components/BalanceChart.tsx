...
export default function BalanceChart({ rows, inflAdj, age55Year, payoutStartYear, showCpfis }: Props) {
  ...
  useEffect(() => {
    ...
    const cpfData = sample.map(r => Math.round(r.cpfis  / adj(r)));

    const age55Idx  = sample.findIndex(r => r.isPost55);
    const payoutIdx = sample.findIndex(r => r.isPayout);
    ...
    if (showCpfis) {
      datasets.push({
        label: 'CPFIS investments',
        data: cpfData,
        backgroundColor: 'rgba(139,92,246,0.72)',  // purple — distinct from all CPF bands
        stack: 's', borderWidth: 0,
      });
    }
        ...
        plugins: {
          ...
          tooltip: {
            ...
            callbacks: {
              ...
              footer: items => `Total incl. CPFIS: $${items.reduce((s, i) => s + (i.raw as number), 0).toLocaleString('en-SG')}`,
            },
          },
        },
        scales: {
  ...
  }, [rows, inflAdj, age55Year, payoutStartYear, showCpfis]);

  return <canvas ref={canvasRef} role="img" aria-label="Stacked bar chart of CPF and CPFIS balances by age" />;
}
